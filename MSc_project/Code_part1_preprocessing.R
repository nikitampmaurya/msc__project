...
################################### step 6: Repeating step for 3,4 and 5 dataset 2 ###################################
...
# visualize thru venn diagram, then you will understand better.
...
# now map ensemble ids of data4_shared with entrez ids, gene symbols, gene biotypes using data1_shared

mapping_info <- dplyr::select(data1_shared, ensembl_gene_id, entrezgene_id, hgnc_symbol, gene_biotype) %>%
  dplyr::distinct()

data4_annotated <- data4_shared %>%
  left_join(mapping_info, by = "ensembl_gene_id")

nrow(data4_annotated) # 25720
nrow(data1_shared) # 25720
...
log_norm_counts_list = list()  # to store log-transformed CPMs (Counts Per Million) for each study after normalization.
...
for (study in studies) {
  ...
  study_counts = master_countdata[, study_samples, drop = FALSE]
  ...
  dge = DGEList(counts = study_counts, group = study_metadata$`Tissue`)
  ...
  study_counts_filtered = study_counts[keep, , drop = FALSE] #  and now filter out low-expressed genes from the count matrix for the current study.
  ...
  filtered_data_list[[study]] <- study_counts_filtered # Stores the filtered count matrix (genes that passed low-expression filtering) for the current study in a separate list.
...
####################################### Step 12: Vislualise before and after filtering and log-normalising datasets #############################
